...
def update_exe():
    ...
    log.write(f'Updating to {Latest_version}\n')
    ...
    log.write(f'Successfully updated from {version} -> {Latest_version}\n')
    logger("Exit: Logging Closed Successfully for 'Transformers Fall Of Cybertron Statistics' application\n")
    log.close()

def update_github():
    import webbrowser
    webbrowser.open('https://github.com/AnmolBhaiyaShah/FOC-statistics-viewer')
    del webbrowser
...
if True:
    ...
    if patch_available:
        button_github     =Button(None, {'text': ' Update from Github ','command':update_github,'cursor':'hand2',Pack:{'side':'right'}})
        button_github     =Button(None, {'text': ' Update from Github ','command':update_github,'cursor':'hand2',Pack:{'side':'left'}})
